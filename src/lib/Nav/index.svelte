<script>
  ...
  $: active = tabs.find(
    (tab) =>
      tab.dest == activeTab ||
      (tab.nest && tab.children.find((subTab) => subTab.dest == activeTab))
  );
...
<svelte:head>
  <title
    >{!activeTab[1] ? "Home" : activeTab[1].toUpperCase() + activeTab.slice(2)} |
    League Page</title
  >
</svelte:head>

<nav>
  <a href="/"><img id="logo" alt="league logo" src="./ebdbLogo.png" /></a>
